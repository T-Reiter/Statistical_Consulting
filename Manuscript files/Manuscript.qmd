...
abstract: |
  ...
  | **Project Partners:** 
  | Prof. Dr. Paul Thurner and Fabian Haggerty 
  | (Geschwister Scholl Institute of Political Science)
  
  | **Date:** 
  | Munich, January 2, 2024
...
# Single grobs for rows (using expressions for hats)
row_header_combined_1 <- textGrob(
  expression(paste(hat(p)[0], "[,1]")),
  x=0, hjust=0, 
  gp=gpar(fontsize=10, lineheight=1)
)
row_header_combined_2 <- textGrob(
  expression(paste(hat(p)[1], "[,1]")),
  x=0, hjust=0,
  gp=gpar(fontsize=10, lineheight=1)
)
row_header_combined_3 <- textGrob(
  paste0("omces[,1]"), 
  x=0, hjust=0,
  gp=gpar(fontsize=10, lineheight=1)
)
