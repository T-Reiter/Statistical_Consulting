...
author:
  - name: "Thomas Reiter"
    affiliations: [stat-consulting]
  - name: "Philipp Sterner"
    affiliations: [stat-consulting]

affiliations:
  - id: stat-consulting
    name: "Statistical Consulting, Master Statistics and Data Science, LMU Munich, Institute of Statistics"

abstract: |
  **Supervisor:** Prof. Dr. David Rügamer

  **Project Partners:** Prof. Dr. Paul Thurner and Fabian Haggerty  
...
format:
  pdf:
    ...
    lot: true
    title-page: true
    title-block-style: fancy
    # Ensure the order: title, authors, affiliations, abstract, date
    title-block-components: [title, author, affiliations, abstract, date]
...
header-includes:
  ...
  - \usepackage{etoolbox}
  - \usepackage{footmisc}
  - \pretocmd{\tableofcontents}{\clearpage\setcounter{page}{2}}{}{}
...
This report explores the application of an advanced statistical model based on Bayesian additive regression trees [*cjbart*\; @robinson_how_2024] model, to analyze effect heterogeneity in high-dimensional conjoint analyses. 
The primary focus is on examining citizens' preferences from five NATO-member countries --- USA, UK, Germany, Italy, and France --- regarding support for Ukraine amid the Russian aggression. 
...
Following cjbart approach, we investigate different nested causal quantities.
The study shows heterogeneity in attribute-level importance influenced by demographic covariates and political opinions, such as participants' nationality, gender, and political attitudes.
This heterogeneity is further explored using interpretable machine learning techniques, such as variable importance measures of random forests and single decision tree partitioning.
For example, UK participants differed to other countries with regard to selection probability changes, particularly regarding the impact of civilian casualties and risks of nuclear strikes. 
Additionally, the attitudes toward general concessions gave rise to differences in selection probabilities.

We discuss methodological caveats, such as robustness of the cjbart model.
We highlight possible issues such as model convergence and the exploratory interpretation of the analysis. 
The report concludes that cjbart is a powerful exploratory tool for high-dimensional conjoint data, offering nuanced insights into heterogeneous treatment effects. 
These findings can be used to generate new hypotheses, informing subsequent confirmatory research and political theory.
...
The AMCE is the effect of a specific attribute level of interest compared to a reference level of the same attribute, holding equal the joint distribution of all other attributes and averaged over this joint distribution as well as the sampling distribution from the population [@robinson_how_2024].
...
Specifically, the OMCE estimates from step 2 are summed and divided by $J \times K$, that is, the number of profiles times the number of rounds (which is the number of total observations per person).
Because the OMCEs result from draws of a posterior distribution, credibility intervals for the IMCE can be constructed by using the empirical quantiles of this distribution.
...
Subsequently, variable importance measures (VIMPs) are derived to assess which covariates are driving the data partitioning. Briefly, the VIMPs are calculated by permuting the covariates (i.e., by perturbing its values).
...
All analyses including a reproducible can be found at \url{https://github.com/T-Reiter/Statistical_Consulting}.
...
However, random forests have been shown to have good performance with default values of its hyperparameters [@strobl2009introduction], so we believe the focus should be on the hyperparameters of the upstream BART model.
...
We complemented our analyses with an exemplary run of the FactorHet method.
The results of this analysis (with a two- and three-cluster solution) can be found in Appendix A6.
...
In the course of the project, our project partner and we noticed that solutions of cjbart differ (slightly) when rerunning the analysis (using different seeds or no prespecified seed on Windows machines).
One possible reason was that the models failed to converge.
To investigate this issue in more detail, we reran the analysis 20 times (i.e., estimating the model with BART and extracting the OMCEs/IMCEs/AMCEs).
The results of this analysis can be found in appendices A1-5.
In A1, the point estimates of AMCEs across the 20 iterations are displayed.
As can be seen, AMCEs do not fluctuate noticeably.
When looking at the ranges of IMCEs per person (A2), we observe that the IMCEs for some attributes fluctuate by around five to seven percentage points (most noticeably for attributes "Territorial cessions of Ukraine to Russia" and "How much political self-determination of Ukraine").
Because we assumed that failed convergence might be an issue, we explored exemplary traceplots, showing the draws from the reference level and the level of interest posterior distribution (the difference of which corresponds to the OMCE).
These traceplots are shown in A3 and A4.
We can see that for none of the displayed chains, the values of draws converge to a stable value which in turn leads to fluctuations in OMCEs.
This is the case for both the default sized model and a larger model with increased burn-in and posterior draws.
Consequently, the model seems to have convergence issues.
This finding is corroborated by the figure in A5, which shows the distribution of p-values for the Geweke's test [@geweke1992evaluating] whether the Markov chain converged (where the null hypothesis can be interpreted as convergence).
Thus, we would be cautious to interpret IMCE results, especially for attributes which show a high range of person-specific IMCEs in A2. 
...
As with any new method, further methodological research is needed to assess the capabilities of the method under different conditions.
...
::: {#amce-robustness style="text-align:left;"}
![Stability of AMCE Point Estimates](figures/Robustness_AMCEs.png)
Note. AMCE point estimates across 20 model fitting iterations using different seed values.\
:::


## A2 IMCE Stability across Multiple Model Fitting Iterations
::: {#imce-robustness style="text-align:left;"}

![Within-Individual Variability of IMCE Estimates](figures/Robustness_IMCEs.png)
...
# Load the images
usk_p0_small <- rasterGrob(
  png::readPNG("figures/Traceplots/Ukr_Sold_killed_phat0_12k_1.png"), 
  interpolate = TRUE, width=unit(1,"npc"), height=unit(1,"npc")
)
usk_p0_large <- rasterGrob(
  png::readPNG("figures/Traceplots/Ukr_Sold_killed_phat0_12k_1_large.png"), 
  interpolate = TRUE, width=unit(1,"npc"), height=unit(1,"npc")
)
usk_p1_small <- rasterGrob(
  png::readPNG("figures/Traceplots/Ukr_Sold_killed_phat1_25k_1.png"), 
  interpolate = TRUE, width=unit(1,"npc"), height=unit(1,"npc")
)
usk_p1_large <- rasterGrob(
  png::readPNG("figures/Traceplots/Ukr_Sold_killed_phat1_25k_1_large.png"), 
  interpolate = TRUE, width=unit(1,"npc"), height=unit(1,"npc")
)
usk_omce_small <- rasterGrob(
  png::readPNG("figures/Traceplots/Ukr_Sold_killed_omce_25k_1.png"), 
  interpolate = TRUE, width=unit(1,"npc"), height=unit(1,"npc")
)
usk_omce_large <- rasterGrob(
  png::readPNG("figures/Traceplots/Ukr_Sold_killed_omce_25k_1_large.png"), 
  interpolate = TRUE, width=unit(1,"npc"), height=unit(1,"npc")
)

# Define column headers (centered)
col_header1 <- textGrob("Default Sized Model", x=0.5, hjust=0.5, gp=gpar(fontface="bold", fontsize=10))
col_subheader1 <- textGrob("Burn-In: 100 \nPosterior Draws: 1000", x=0.5, hjust=0.5, gp=gpar(fontsize=8))

col_header2 <- textGrob("Large Model", x=0.5, hjust=0.5, gp=gpar(fontface="bold", fontsize=10))
col_subheader2 <- textGrob("Burn-In: 2000 \nPosterior Draws: 5000", x=0.5, hjust=0.5, gp=gpar(fontsize=8))

col_header1_combined <- arrangeGrob(col_header1, col_subheader1, ncol=1,
                                    heights=c(unit(0.5,"lines"), unit(0.5,"lines")))
col_header2_combined <- arrangeGrob(col_header2, col_subheader2, ncol=1,
                                    heights=c(unit(0.5,"lines"), unit(0.5,"lines")))

# Single grobs for rows with newline and adjusted lineheight
row_header_combined_1 <- textGrob(paste0("$p^{hat}_{0}$\n",
                                         "phat_0[,1]"), x=0, hjust=0, 
                                  gp=gpar(fontsize=10, lineheight=1))
row_header_combined_2 <- textGrob(paste0("p_hat_1\n",
                                         "phat_0[,1]"), x=0, hjust=0,
                                  gp=gpar(fontsize=10, lineheight=1))
row_header_combined_3 <- textGrob(paste0("omce_1\n",
                                         "omces[,1]"), x=0, hjust=0,
                                  gp=gpar(fontsize=10, lineheight=1))

grid.arrange(
  textGrob(""), col_header1_combined, col_header2_combined,
  row_header_combined_1, usk_p0_small, usk_p0_large,
  row_header_combined_2, usk_p1_small, usk_p1_large,
  row_header_combined_3, usk_omce_small, usk_omce_large,
  ncol=3,
  widths=c(1,4,4),
  heights=c(1.5,3,3,3)
)
...
```{r convergence-diagnostics-sov, echo=FALSE, fig.align='center', fig.fullwidth=TRUE, out.width="100%"}
#| fig-subcap: "Exemplary Posterior Draw Traceplots for Convergence Assessment - Attribute 9"
...
#| label: convergence-diagnostics-sov
# if needed add linebreaks in fig-cap via \\linebreak
...
# Load the images
sov_p0_small <- rasterGrob(
  png::readPNG("figures/Traceplots/Sovereignity_phat0_Full_80088.png"), 
  interpolate = TRUE, width=unit(1,"npc"), height=unit(1,"npc")
)
sov_p0_large <- rasterGrob(
  png::readPNG("figures/Traceplots/Sovereignity_phat0_Full_80088_large.png"), 
  interpolate = TRUE, width=unit(1,"npc"), height=unit(1,"npc")
)
sov_p1_small <- rasterGrob(
  png::readPNG("figures/Traceplots/Sovereignity_phat1_EU_NATO_80088.png"), 
  interpolate = TRUE, width=unit(1,"npc"), height=unit(1,"npc")
)
sov_p1_large <- rasterGrob(
  png::readPNG("figures/Traceplots/Sovereignity_phat1_EU_NATO_80088_large.png"), 
  interpolate = TRUE, width=unit(1,"npc"), height=unit(1,"npc")
)
sov_omce_small <- rasterGrob(
  png::readPNG("figures/Traceplots/Sovereignity_omce_EU_NATO_80088.png"), 
  interpolate = TRUE, width=unit(1,"npc"), height=unit(1,"npc")
)
sov_omce_large <- rasterGrob(
  png::readPNG("figures/Traceplots/Sovereignity_omce_EU_NATO_80088_large.png"), 
  interpolate = TRUE, width=unit(1,"npc"), height=unit(1,"npc")
)
...
# Single grobs for rows with newline and adjusted lineheight
row_header_combined_1 <- textGrob(paste0("p_hat_0\n",
                                         "phat_0[,1]"), x=0, hjust=0, 
                                  gp=gpar(fontsize=10, lineheight=1))
row_header_combined_2 <- textGrob(paste0("p_hat_1\n",
                                         "phat_0[,1]"), x=0, hjust=0,
...
grid.arrange(
  textGrob(""), col_header1_combined, col_header2_combined,
  row_header_combined_1, sov_p0_small, sov_p0_large,
  row_header_combined_2, sov_p1_small, sov_p1_large,
  row_header_combined_3, sov_omce_small, sov_omce_large,
  ncol=3,
  widths=c(1,4,4),
  heights=c(1.5,3,3,3)
)
...
```{r convergence-diagnostics-geweke, echo=FALSE, fig.align='center', fig.fullwidth=TRUE, out.width="100%"}
#| fig-subcap: "P-Value Distribution of Geweke's Diagnostic Test for 100 Sample MCMC Draws"
#| fig-cap: "Note. Images display the p-value distribution of Geweke's Convergence Diagnostic Test (as implemented in r package BART) applied to a random sample of 100 Random Markov Chains as generated using the BART model with larger-than-default values for Burn-In and Posterior Draws. Geweke's Test is used for diagnosing Markov Chain convergence via testing equality of means of the 10 and last 50 percent (default values) of the posterior draws."
#| label: convergence-diagnostics-geweke
# if needed add linebreaks in fig-cap via \\linebreak
...
# Load the images
usk_gew_p0 <- rasterGrob(
  png::readPNG("figures/Geweke_pvals/Ukr_Sold_killed_phat0_12k_large.png"), 
  interpolate = TRUE, width=unit(1,"npc"), height=unit(1,"npc")
)
usk_gew_p1 <- rasterGrob(
  png::readPNG("figures/Geweke_pvals/Ukr_Sold_killed_phat1_25k_large.png"), 
  interpolate = TRUE, width=unit(1,"npc"), height=unit(1,"npc")
)
usk_gew_omce <- rasterGrob(
  png::readPNG("figures/Geweke_pvals/Ukr_Sold_killed_omce_25k_large.png"), 
  interpolate = TRUE, width=unit(1,"npc"), height=unit(1,"npc")
)
sov_gew_p0 <- rasterGrob(
  png::readPNG("figures/Geweke_pvals/Sovereignity_phat0_Full_large.png"), 
  interpolate = TRUE, width=unit(1,"npc"), height=unit(1,"npc")
)
sov_gew_p1 <- rasterGrob(
  png::readPNG("figures/Geweke_pvals/Sovereignity_phat1_EU_NATO_large.png"), 
  interpolate = TRUE, width=unit(1,"npc"), height=unit(1,"npc")
)
sov_gew_omce <- rasterGrob(
  png::readPNG("figures/Geweke_pvals/Sovereignity_omce_EU_NATO_large.png"), 
  interpolate = TRUE, width=unit(1,"npc"), height=unit(1,"npc")
)

# Define column headers (centered)
col_header1 <- textGrob("Attribute 1\nUkrainian Military Casualties", x=0.5, hjust=0.5, gp=gpar(fontface="bold", fontsize=10))
col_subheader1 <- textGrob("Large Model\nBurn-In: 2000 \nPosterior Draws: 5000", x=0.5, hjust=0.5, gp=gpar(fontsize=8))

col_header2 <- textGrob("Attribute 9\nSovereignity", x=0.5, hjust=0.5, gp=gpar(fontface="bold", fontsize=10))
col_subheader2 <- textGrob("Large Model\nBurn-In: 2000 \nPosterior Draws: 5000", x=0.5, hjust=0.5, gp=gpar(fontsize=8))
...
grid.arrange(
  textGrob(""), col_header1_combined, col_header2_combined,
  row_header_combined_1, usk_gew_p0, sov_gew_p0,
  row_header_combined_2, usk_gew_p1, sov_gew_p1,
  row_header_combined_3, usk_gew_omce, sov_gew_omce,
  ncol=3,
  widths=c(1,4,4),
  heights=c(3,3,3,3)
)
...
![FactorHet 2-Cluster Solution](figures/2024-12-06_FactorHet_2C_AMCEs.png)
...
![FactorHet 3-Cluster Solution](figures/2024-12-06_FactorHet_3C_AMCEs.png)
